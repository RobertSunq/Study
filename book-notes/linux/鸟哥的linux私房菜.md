...
![0008](static\picture\0008.png)

+ 第一个字符代表该文件的类型
  + 『 **d** 』表示：目录
  + 『 **-**  』表示：文件
  + 『 **I** 』表示：链接文件(link file)
  + 『 **b** 』表示：装置文件里面的可供储存的接口设备(可随机存取装置)
  + 『 **c** 』表示：装置文件里面的串行端口设备，例如键盘、鼠标(一次性读取装置)

+ 接下来的字符，三个为一组，表示用户、组群、其他人对该文件权限
  ...
  + 『 **x** 』表示：可执行 (execute)
    ...
    > x： (0b)100
    >
    > 因为所有的权限编码都可以用一个三位二进制数字求和表示：
    >
    > rwx： (0b)001 + (0b) 010 + (0b)100 =  (0b)111 = 7
